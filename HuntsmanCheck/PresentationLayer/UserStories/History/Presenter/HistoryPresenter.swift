...
extension HistoryPresenter: HistoryInteractorOutput {
    ...
    func didUploadBlk() {
        view.hideLoading()
        view.showSuccess(message: L10n.History.successfullyUpload)
    }
}
...
extension HistoryPresenter {
    ...
    private func updateTickets() {
        ...
        sections.sort { (key1: TableSection, key2: TableSection) in
            let date1 = formatter.date(from: key1.title ?? "", by: .RUSSIAN_DATE) ?? Date()
            let date2 = formatter.date(from: key2.title ?? "", by: .RUSSIAN_DATE) ?? Date()
